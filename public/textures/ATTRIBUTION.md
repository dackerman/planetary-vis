# Solar System Scope textures

Source: https://www.solarsystemscope.com/textures/

Creator: Solar System Scope / INOVE

License: Creative Commons Attribution 4.0 International (CC BY 4.0)
https://creativecommons.org/licenses/by/4.0/

Suggested exhibit credit:
"Planet textures: Solar System Scope / INOVE, CC BY 4.0."
Link "Solar System Scope" to the source page and "CC BY 4.0" to the license.
If resizing, recoloring, or otherwise modifying these downloaded files, indicate those changes alongside the credit.

The source describes the planet maps as equirectangular, based on NASA elevation and imagery data, with colors informed by spacecraft imagery. It notes that unmapped gaps may contain fictional terrain and that colors are somewhat more saturated than reality.

Downloads are unmodified. All requested assets were available on 2026-09-14.

All 2k planet maps: 2048 x 1024 JPEG.
8k Earth day and cloud maps: 8192 x 4096 JPEG.
Saturn ring: 2048 x 125 RGBA PNG, with transparency.
Earth cloud maps are opaque JPEGs: use luminance as an alpha mask rather than expecting embedded alpha.

## Files and direct source URLs

- 2k_mercury.jpg: https://www.solarsystemscope.com/textures/download/2k_mercury.jpg
- 2k_venus_atmosphere.jpg: https://www.solarsystemscope.com/textures/download/2k_venus_atmosphere.jpg
- 2k_mars.jpg: https://www.solarsystemscope.com/textures/download/2k_mars.jpg
- 2k_jupiter.jpg: https://www.solarsystemscope.com/textures/download/2k_jupiter.jpg
- 2k_saturn.jpg: https://www.solarsystemscope.com/textures/download/2k_saturn.jpg
- 2k_saturn_ring_alpha.png: https://www.solarsystemscope.com/textures/download/2k_saturn_ring_alpha.png
- 2k_uranus.jpg: https://www.solarsystemscope.com/textures/download/2k_uranus.jpg
- 2k_neptune.jpg: https://www.solarsystemscope.com/textures/download/2k_neptune.jpg
- 2k_earth_daymap.jpg: https://www.solarsystemscope.com/textures/download/2k_earth_daymap.jpg
- 8k_earth_daymap.jpg: https://www.solarsystemscope.com/textures/download/8k_earth_daymap.jpg
- 2k_earth_clouds.jpg: https://www.solarsystemscope.com/textures/download/2k_earth_clouds.jpg
- 8k_earth_clouds.jpg: https://www.solarsystemscope.com/textures/download/8k_earth_clouds.jpg
- 2k_sun.jpg: https://www.solarsystemscope.com/textures/download/2k_sun.jpg

Sun upgrade: Solar System Scope / INOVE, CC BY 4.0. Downloaded from https://www.solarsystemscope.com/textures/download/8k_sun.jpg ; actual supplied resolution is 4096 × 2048 (stored as 4k_sun.jpg). Animated fine surface detail and plasma loops are procedural illustrations.
