...
export interface Telemetry { gridKm: number; movingKms: number; traveledKm: number; referenceKm: number; descent?: { progress: number; radiusRatio: number; fade: number; paused: boolean }; blackHoleKm?: number; blackHoleRadiusRatio?: number; shader?: { width: number; height: number; fps: number }; }
